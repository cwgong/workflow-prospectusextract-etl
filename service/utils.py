...
def request_pdf_always(start_at, end_at, time_field, ann_group, extract_method):
    logging.info('开始查询数据')
    ...
    data = db.get_pdf(start_at, end_at, time_field, ann_group)
    data_list = []
    count = 0
...
def handle_data_list(data_list, extract_method):
    ...
    for i in range(len(data_list)):
        ...
        if parse_status != '1':
            logging.info('parse not finish: ' + sec_code + ' title: ' + title)
            continue

        if '摘要' in title or '确认意见' in title or '更正公告' in title:
            logging.info('不符合要求的title, continue')
            continue

        mar_type = item.get('marType')
        if len(mar_type) == 0 or len(sec_code) == 0:
            logging.info('mar type or sec code is empty, continue')
            continue
        ...
        if len(sec_name) == 0:
            logging.info('sec name is empty, continue')
            continue
        entity_id = request_entity_id(sec_name)
        if len(entity_id) == 0:
            logging.info('cannot get entity_id, sec_name: ' + sec_name + ', continue')
            continue

        detail = item.get('detail')
        # 1.抽取
        logging.info('extracting...' + title + item.get('annUrl'))
        text_knowledges, table_knowledges = extract_method(detail, sec_name)
...
def package_text_knowledge(pdf_item, entity_id, text_knowledges):
    text_id = pdf_item.get('textId')  # annDetailTW
    if text_id is None:
        text_id = pdf_item.get('textid')  # annDetail
    evidence_id = str(text_id)
